...
#ifndef SHARE_VM_MEMORY_ALLOCATION_HPP_
#define SHARE_VM_MEMORY_ALLOCATION_HPP_
...
public:
	_NOINLINE_ void* operator new(size_t size, address caller_pc = 0) throw ();_NOINLINE_ void* operator new(
			size_t size, const std::nothrow_t& nothrow_constant,
			address caller_pc = 0) throw ();_NOINLINE_ void* operator new [](
			size_t size, address caller_pc = 0) throw ();_NOINLINE_ void* operator new [](
			size_t size, const std::nothrow_t& nothrow_constant,
...
#define METASPACE_OBJ_TYPE_NAME_CASE(name) case name ## Type: return #name;
	...
	void* operator new(size_t size, ClassLoaderData* loader_data,
			size_t word_size, bool read_only, Type type,
			Thread* thread) throw ();
...
public:
	void* operator new(size_t size, AllocFailType alloc_failmode,
			size_t length) throw ();
	void operator delete(void* p);
	Chunk(size_t length);
	...
	static size_t aligned_overhead_size(void) {
		return ARENA_ALIGN(sizeof(Chunk));
	}
	static size_t aligned_overhead_size(size_t byte_size) {
		return ARENA_ALIGN(byte_size);
	}
...
protected:
	...
	void* grow(size_t x, AllocFailType alloc_failmode =
			AllocFailStrategy::EXIT_OOM);
	size_t _size_in_bytes;    // Size of arena (used for native memory tracking)

	NOT_PRODUCT(static julong _bytes_allocated;) // total #bytes allocated since start
	friend class AllocStats;debug_only(void* malloc(size_t size);)
...
public:
	...
	void *Amalloc_4(size_t x, AllocFailType alloc_failmode =
			AllocFailStrategy::EXIT_OOM) {
	...
	void* Amalloc_D(size_t x, AllocFailType alloc_failmode =
			AllocFailStrategy::EXIT_OOM) {
		assert( (x&(sizeof(char*)-1)) == 0, "misaligned size" );
		debug_only(if (UseMallocOnly) return malloc(x);)
...
#define DALIGN_M1 7
	...
	static void free_malloced_objects(Chunk* chunk, char* hwm, char* max,
			char* hwm2) PRODUCT_RETURN;
...
extern char* resource_allocate_bytes(Thread* thread, size_t size,
		AllocFailType alloc_failmode = AllocFailStrategy::EXIT_OOM);
extern char* resource_reallocate_bytes(char *old, size_t old_size,
		size_t new_size, AllocFailType alloc_failmode =
				AllocFailStrategy::EXIT_OOM);
