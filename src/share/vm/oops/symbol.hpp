...
public:
	...
	// Tests if the symbol stars with the give prefix
	bool starts_with(const char* prefix, int len) const;
	bool starts_with(const char* prefix) const {
		return starts_with(prefix, (int) strlen(prefix));
	}

	// Tests if the symbol starts with the give prefix
	int index_of_at(int i, const char* str, int len) const;
	int index_of_at(int i, const char* str) const {
		return index_of_at(i, str, (int) strlen(str));
	}

	// Three-way compare for sorting; return -1/0/1 if receiver is </==/> than arg
	...
	// Treating this symbol as a class name, return the Java name for the class.
...
// Since Symbol*s are in C_HEAP, their relative order in memory neven changes,
// so use address comparison for speed
int Symbol::fast_compare(Symbol* other) const {
	return ( ( (uintptr_t)this < (uintptr_t)other ) ? -1 : ( (uintptr_t)this == (uintptr_t)other ) ? 0 : 1);
}
